...
const HighlightMatch = React.memo(({ text, query }) => {
  ...
  return (
    <span>
      {parts.map((part, i) =>
        regex.test(part) ? (
          <span key={i} className="bg-primary/30 text-primary-hover font-medium rounded-sm px-0.5">{part}</span>
        ) : (
...
const TopSearch = () => {
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-md">
      <div className={`flex items-center gap-2 bg-surface px-3 py-2 rounded-md border transition-colors ${isOpen ? 'border-primary ring-1 ring-primary/50' : 'border-border'}`}>
          ...
          {filteredResults.length > 0 ? (
            <ul className="py-1">
              {filteredResults.map((result, index) => (
                <li key={result} className={`px-4 py-2 text-sm cursor-pointer ${index === selectedIndex ? 'bg-surface-hover text-text' : 'text-text-muted hover:bg-surface-hover hover:text-text'}`}
                  onClick={() => { setQuery(result); setIsOpen(false); }}
