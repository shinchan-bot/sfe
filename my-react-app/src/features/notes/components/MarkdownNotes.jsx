...
const MarkdownNotes = () => {
  ...
  useEffect(() => {
    if (isSaving) {
      const timer = setTimeout(() => { dispatch(saveComplete()); }, 1000);
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 text-sm text-text-muted">
          {isSaving ? (
            ...
            <><Save size={14} /><span>{lastSaved ? `Saved ${lastSaved}` : 'All changes saved'}</span></>
          )}
        </div>
      </div>
      <div className="flex-1 grid grid-cols-2 gap-4 h-full min-h-0">
        <div className="h-full border border-border rounded-lg overflow-hidden bg-surface-hover/30 focus-within:border-primary transition-colors">
          <textarea value={content} onChange={handleChange} className="w-full h-full bg-transparent p-4 text-sm text-text outline-none resize-none scrollbar-thin" placeholder="Write markdown here..." />
        </div>
