...
import { addMessage, setTyping, ChatMessage } from '../chatSlice';
    ...
    <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} mb-4`}>
      <div 
        className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm
          ${isMe ? 'bg-primary text-white rounded-br-none' : 'bg-surface-hover text-text rounded-bl-none border border-border'}
        `}
      >
        {msg.text}
      </div>
...
const ChatPanel = () => {
  ...
  // Mock WebSocket for incoming messages
  useEffect(() => {
    const interval = setInterval(() => {
      // 10% chance to simulate someone typing
      if (Math.random() > 0.9) {
        ...
        setTimeout(() => {
          ...
          dispatch(addMessage({
            ...
            sender: 'other',
            text: 'Just testing the mock websocket!',
            timestamp: Date.now(),
          }));
        }, 2000);
      }
    ...
    <div className="h-full flex flex-col">
      ...
      <form onSubmit={handleSend} className="mt-auto relative">
        <input
          ...
          className="w-full bg-surface-hover border border-border rounded-xl py-3 pl-4 pr-12 text-sm text-text outline-none focus:border-primary transition-colors placeholder:text-text-muted"
        />
        <button 
          type="submit" 
          disabled={!text.trim()}
